...
% Analysis class for a 3-dimensional framed structure
    ...
    methods (Access = private)
        ...
        function ComputeError(self, KFF, KFN, freeDOF, psDOF, PF,FEF_s)
            ...
            % Create a column vector of all deflections
            full_d = dg(:);

            % Back calculate load vector.
            % Equation: PF = KFF*DF + KFN*DN + FEFS
            PF_check = KFF*full_d(freeDOF)+KFN*full_d(psDOF);

            % Check percent error between given load vector and
